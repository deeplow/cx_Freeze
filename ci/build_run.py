"""Helper module to run tests on Linux, Windows and macOS. Also works with
...
def get_command_line(command: Tuple[Union[Path, str], ...]) -> str:
    return " ".join(list(map(os.fspath, command)))


@contextlib.contextmanager
def pushd(target: Union[Path, str]):
    saved = os.getcwd()
    os.chdir(target)
    try:
        yield saved
...
async def run_svc(command: Tuple[str, ...], output: str) -> Process:
    process = await asyncio.create_subprocess_exec(
        *command, stdout=PIPE, stderr=PIPE
    )
    return process
